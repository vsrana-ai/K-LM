# -*- encoding:utf-8 -*-
"""
  This script is to perform classification task along with ranking the triples
"""
import sys
import torch
import json
    ...
    for line_id, line in enumerate(sentences): 
        ...
        if len(line) == 2:
            ...
            tokens, pos, vm, _ = kg.add_knowledge_with_vm(
                [text], add_pad=True, max_length=args.seq_length
            )
            tokens = tokens[0]
            pos = pos[0]
            vm = vm[0].astype("bool")
...
def main():
    ...
    parser.add_argument(
        "--train_path", type=str, required=True, help="Path of the trainset."
    )
    parser.add_argument(
        "--dev_path", type=str, required=True, help="Path of the devset."
    )
    parser.add_argument(
        "--test_path", type=str, required=True, help="Path of the testset."
    )
    parser.add_argument(
        "--config_path",
    ...
    parser.add_argument("--seq_length", type=int,
                        default=256, help="Sequence length.")
    parser.add_argument(
        "--encoder",
        choices=[
            "bert",
    ...
    parser.add_argument("--warmup", type=float,
                        default=0.1, help="Warm up value.")
    ...
    parser.add_argument(
        "--no_vm", action="store_true", help="Disable the visible_matrix"
    )
    ...
    if args.pretrained_model_path is not None:
        ...
        model.load_state_dict(torch.load(
            args.pretrained_model_path), strict=False)
    else:
    ...
    def batch_loader(batch_size, index_ids, input_ids, label_ids, mask_ids, pos_ids, vms): 
        ...
        if instances_num > instances_num // batch_size * batch_size:
            index_ids_batch = index_ids[instances_num //
                                        batch_size * batch_size:]
            input_ids_batch = input_ids[instances_num //
                                        batch_size * batch_size:, :]
            label_ids_batch = label_ids[instances_num //
                                        batch_size * batch_size:]
            mask_ids_batch = mask_ids[instances_num //
                                      batch_size * batch_size:, :]
            pos_ids_batch = pos_ids[instances_num //
                                    batch_size * batch_size:, :]
    ...
    def read_dataset(path, workers_num=1):
        ...
        print(
            "There are {} sentence in total. We use {} processes to inject knowledge into sentences.".format(
    ...
    def evaluate(args, is_test, metrics="Acc"):
        if is_test:
            dataset = read_dataset(
                args.test_path, workers_num=args.workers_num)
        ...
        confusion = torch.zeros(
            args.labels_num, args.labels_num, dtype=torch.long)
    ...
    print("The Used KG are", kg.useable_triples.most_common())  
    print("The Length of Used KG is", len(kg.useable_triples))  

    most_common_subs = kg.useable_triples.most_common()[:5] 
    print("The Injected KG are", [(i, kg.injected_knowledge[i]) for (i, count) in most_common_subs])
    ...
    index_ids = torch.LongTensor([i for i, sample in enumerate(trainset)]) 
    print("input_ids")
    ...
    for epoch in range(1, args.epochs_num + 1):
        ...
        for i, (
            ...
            batch_loader(batch_size, index_ids, input_ids,
                         label_ids, mask_ids, pos_ids, vms)
        ):
            model.zero_grad()
